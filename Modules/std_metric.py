...
class Fairness:
    ...
    def compute(self, original_train, rec_mapped_train, orig_mapped_train, original_test, rec_mapped_test,
        ...
        for dt, m_train, m_test in zip(["Org-Mapped", "Rec-Mapped"], [orig_mapped_train, rec_mapped_train],
            ...
            # Predict and compute metrics
            self.fairness_results(x=o_ts_x, y=o_ts_y, rs=rel_sens_columns_test, s=orig_sens_columns_test,
                                  g_index=test_org_index,
                                  rg_index=test_rel_index, y_name=self.decision_name, type_="-",
            ...
            # Predict and compute metrics
            self.fairness_results(x=m_ts_x, y=o_ts_y, rs=rel_sens_columns_test, s=orig_sens_columns_test,
                                  g_index=test_org_index,
                                  rg_index=test_rel_index, y_name=self.decision_name, type_="-",
...
class FairnessSimplified:
    """ Compute utility metrics
     - Compute Demographic parity
     - Compute Equalized odds
     """

    def __init__(self, decision_name, group_name, orig_group_names, enc_decision_names, enc_group_names,
                 enc_orig_group_names_list, enc_to_drop=[], seed=42):
        self.classifiers = clfrs.Classifiers(seed=seed)
        self.decision_name = decision_name
        self.group_name = group_name
        self.orig_group_names = orig_group_names
        self.enc_decision_names = enc_decision_names
        self.enc_group_names = enc_group_names
        self.enc_orig_group_names_list = enc_orig_group_names_list
        self.enc_to_drop = enc_to_drop
        self.visualise = Visualisation.Visualize(seed=seed)
        self.results = None
    ...
    def train_models(self, x, y):
        ...
        try:
            ...
            print(ve)
            print("Continuing with np.Nan as predicted value.")

    def split(self, data):
        to_drop = self.enc_to_drop[:]
        to_drop.extend(self.enc_group_names)
        to_drop.extend(self.enc_decision_names)
        x = data.drop(to_drop, axis=1)
        y = data[self.enc_decision_names]
        s = data[self.enc_group_names]
        s_orig = [data[cols] for cols in self.enc_orig_group_names_list]
        return x, y, s, s_orig
    ...
    def dirty_from_dummies(self, df, col_name, prefix_sex="="):
        """ Get the column name as argmax quickly """
        cols = [v.split(prefix_sex)[-1] for v in df.columns]
        g = pd.DataFrame(df.values.argmax(1), columns=[col_name])
        for v in g[col_name].value_counts().index:
            g.loc[g[col_name] == v, col_name] = cols[v]
        return g.values

    def fairness_results(self, x, y, g, grp_name, s_org, s_org_name, y_name, type_, scn, result_type):
        """ Compute the fairness results using input x, target y, sensitive attributes s and group index g and
        the relabelled group index rg """

        all_sensitives = [g] + s_org[:]
        all_sensitives_names = [grp_name] + s_org_name[:]

        def shape_sensitive(group, name):
            gr = group.values
            if group.shape[1] > 1:
                gr = self.dirty_from_dummies(group, name, )
                # gr = group.values.argmax(1)
            return pd.DataFrame(gr, columns=[name])
        ...
        for clf_name, pred_y in preds_y.items():
            ...
            self.add_results(Attribute=y_name, Metric="Accuracy", Classifier=clf_name, Type=type_, Scenario=scn,
                             Result=correct_pred.mean(), GroupValue="-", GroupName="AllGroups", ResultType=result_type)
            # Group by sensitive attribute
            for s_values, s_name in zip(all_sensitives, all_sensitives_names):
                # Shape the sensitive values
                g = shape_sensitive(s_values, s_name)
                # Compute accuracy per group
                self.positive_rates(correct_pred, g, s_name, scn, type_, y_name, clf_name, result_type, pos_y_value=1,
                                    metric="g. Accuracy")
                # Compute base rate
                ...
                # Compute tp_fp
                self.tp_and_fp(pred_y, y, g, s_name, scn, type_, y_name, clf_name, result_type, pos_y_value=1)
        preds_y.update({"TargetY": y,
                        grp_name: g[g.columns[0]].values.tolist(),})
        frame = pd.DataFrame.from_dict(preds_y)
        frame["Type"] = type_
        frame["Scenario"] = scn
        frame["ResultType"] = result_type
        return frame

    def compute(self, original_train, rec_mapped_train, orig_mapped_train, original_test, rec_mapped_test,
                orig_mapped_test, exp_folders, mapped_decision=False):
        """ Compute fairness metrics for each scenario """
        o_tr_x, o_tr_y, o_tr_s_grp, o_tr_s_org = self.split(original_train)
        o_ts_x, o_ts_y, o_ts_s_grp, o_ts_s_org = self.split(original_test)
        viz_data = o_ts_x
        if mapped_decision:
            viz_data = pd.concat([o_ts_x, o_ts_y], axis=1)
        # self.visualise.clusters_plots(dFrame=viz_data, labels=o_ts_s_grp.values.argmax(1),
        #                               savefig="{}/Original-Cluster.png".format(exp_folders.fig_dir))
        # Normally o_tr_s == m_tr_s and o_ts_s == m_ts_s == encoded(sens_columns)
        ...
        for dt, m_train, m_test in zip(["Org-Mapped", "Rec-Mapped"], [orig_mapped_train, rec_mapped_train],
                                       [orig_mapped_test, rec_mapped_test]):
            # Extract group information and y.
            m_tr_x, m_tr_y, m_tr_s_grp, m_tr_s_org = self.split(m_train)
            m_ts_x, m_ts_y, m_ts_s_grp, m_ts_s_org = self.split(m_test)
            viz_data = m_ts_x
            if mapped_decision:
                viz_data = pd.concat([m_ts_x, m_ts_y], axis=1)
            # self.visualise.clusters_plots(dFrame=viz_data, labels=test_rel_index,
            #                               savefig="{}/{}-Rel-Cluster.png".format(exp_folders.fig_dir, dt))
            # self.visualise.clusters_plots(dFrame=viz_data, labels=m_ts_s_grp.values.argmax(1),
            #                               savefig="{}/{}-O.Idx-Cluster.png".format(exp_folders.fig_dir, dt))
            # Data publishing: Train on modified, test on modified
            self.train_models(m_tr_x, m_tr_y)
            # Predict and compute metrics
            preds_y = self.fairness_results(x=m_ts_x, y=m_ts_y, g=m_ts_s_grp, grp_name=self.group_name,
                                            s_org=m_ts_s_org, s_org_name=self.orig_group_names,
                                            y_name=self.decision_name, type_="-", scn="Data Pub.", result_type=dt)
            predictions = pd.concat([predictions, preds_y], axis=0)
            # Fair classifier: train on sanitized data and predict using original data
            # self.train_models(m_tr_x, m_tr_y)  # Models are already trained. So no need to do it again.
            # Predict and compute metrics
            preds_y = self.fairness_results(x=o_ts_x, y=o_ts_y, g=m_ts_s_grp, grp_name=self.group_name,
                                            s_org=m_ts_s_org, s_org_name=self.orig_group_names,
                                            y_name=self.decision_name, type_="-", scn="Fair Clfs", result_type=dt)
            predictions = pd.concat([predictions, preds_y], axis=0)
            ...
            self.train_models(m_tr_x, o_tr_y)
            # Predict and compute metrics
            preds_y = self.fairness_results(x=m_ts_x, y=o_ts_y, g=m_ts_s_grp, grp_name=self.group_name,
                                            s_org=m_ts_s_org, s_org_name=self.orig_group_names,
                                            y_name=self.decision_name, type_="-", scn="SOA", result_type=dt)
            predictions = pd.concat([predictions, preds_y], axis=0)
            ...
            preds_y = self.fairness_results(x=m_ts_x, y=o_ts_y, g=m_ts_s_grp, grp_name=self.group_name,
                                            s_org=m_ts_s_org,
                                            s_org_name=self.orig_group_names, y_name=self.decision_name, type_="-",
                                            scn="Local San.", result_type=dt)
            predictions = pd.concat([predictions, preds_y], axis=0)
            preds_y = self.fairness_results(x=m_ts_x, y=m_ts_y, g=m_ts_s_grp, grp_name=self.group_name,
                                            s_org=m_ts_s_org,
        ...
        # Baseline: Train on original, test on original data
        # self.train_models(o_tr_x, o_tr_y)  # Already trained in the local san scenario.
        preds_y = self.fairness_results(x=o_ts_x, y=o_ts_y, g=o_ts_s_grp, grp_name=self.group_name, s_org=m_ts_s_org,
                                        s_org_name=self.orig_group_names, y_name=self.decision_name, type_="-",
                                        scn="Baseline", result_type="Baseline")
        predictions = pd.concat([predictions, preds_y], axis=0)

        predictions.to_csv("{}/predictions.csv".format(exp_folders.results_dir), index=False)
