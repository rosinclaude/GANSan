    ...
    def __encoded_features_format__(self):
        ...
        if self.encoded_features_order is not None:
            ...
            if len(missing_columns):
                missing_df = pd.DataFrame(np.ones((self.df.shape[0], len(missing_columns))) * self.scale,
                                          columns=missing_columns)
                self.df = pd.concat([self.df, missing_df], axis=1, sort=True)
...
class TorchGeneralDataset(TorchCommon):
    ...
    def __init__(self, csv, target_features, xTensor=torch.FloatTensor, yTensor=torch.LongTensor, transform=None,
                 to_drop=None, drop_target=True, drop_from_x_twin=None, prefix_sep="="):
        """
        ...
        :param drop_from_x_twin: Do not drop these target from x, create an additional variable x_twin_dropped that will
        correspond to x - drop_from_x_twin variables: [x[0:drop_from_x_twin], x[drop_from_x_twin+1, -1]];
         x_twin_drp is x with fake_drop columns dropped
        ...
        self.x_twin_drp = None
        if drop_from_x_twin is not None:
            drop.extend(drop_from_x_twin)
            self.x_twin_drp = \
                xTensor(df.drop(self.__filter_features___(df, drop, prefix_sep=prefix_sep), axis=1).values)
    ...
    #     x = self.x[index]
    #     x_wo_fake = self.x_wo_fake[index]
    #     yf = self.yf[index]
    #     y = {}
    #     yArg = {}
    ...
    #     xfd_yn = self.cat_noise(self.cat_target(x_wo_fake, yf))
    #     # sample = {'x': x, "xyn": xyn, "yf": yf, "yArg": torch.cat((list(yArg.values()))),
    #     #           **y, **yArg, }
    #     sample = {'x': x, "xfd_yn": xfd_yn, "yf": yf, **y, **yArg, }
    #     return sample

    def __getitem__(self, index):
        x = self.x[index]
        sample = {'x': x}
        y_float = self.yf[index]
        sample.update({'y_float': y_float})
        y = {}
        # y_argmax = {}
        for i, t in enumerate(self.y):
            y.update({"y_{}".format(i): t[index]})
            # y_argmax.update({"y_{}_argmax".format(i): self.arg_max(t[index])})
        sample.update(**y)
        if self.x_twin_drp is not None:
            x_twin_drp = self.x_twin_drp[index]
            sample.update({"x_twin_drp": x_twin_drp})
        # if self.noise is not None and self.noise > 0:
        #     sample.update({"x_n_noise": self.cat_noise(x)})
        #     if self.x_twin_drp is not None:
        #         sample.update({"x_twin_drp_n_noise": self.cat_noise(x_twin_drp)})
        # x_n_noise = self.cat_noise(self.cat_target(x_wo_fake, y_float))
        return sample
